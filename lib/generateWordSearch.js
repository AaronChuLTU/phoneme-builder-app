...
<style>
  /* The puzzle follows the student's device preference, not the teacher's
     builder theme — see the note in generateWordle.js. */
  :root {
    color-scheme: light;
    --bg: #f6f7f9;
    --surface: #ffffff;
  ...
  * { box-sizing: border-box; }
  ...
  @media print {
    ...
    /* Printing always uses the light palette regardless of screen theme —
       a dark grid would be unreadable and waste toner. */
    :root {
      --bg: #ffffff; --surface: #ffffff; --text: #000000;
      --border: #999999; --muted: #444444;
    }
    body { background: #fff; }
  }
</style>
...
(function () {
  ...
  function buildList() {
    ...
    for (var i = 0; i < PLACEMENTS.length; i++) {
      ...
      // Indexed rather than keyed by the word itself: teachers can type any
      // word, and a quote character would break a querySelector built by
  ...
  function markFound(p) {
    ...
    var index = PLACEMENTS.indexOf(p);
    var items = listEl.children;
    if (index >= 0 && items[index]) items[index].classList.add("done");
